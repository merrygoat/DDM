...
def loadimages(num_images, analysis_radius, image_directory, file_prefix, file_suffix, z_slices):
    ...
    ftimagelist = np.memmap(tmp_file, mode='w+', dtype=np.complex128,
                            shape=(num_files, num_slices, analysis_radius*2, analysis_radius))

	for slice in range(z_slices):
		for timestep in range(num_files):
			tmp_image = Image.open(image_directory + file_prefix + "_t" + '{:03d}'.format(timestep) + "_z" + '{:03d}'.format(slice) + file_suffix)
			...
			ftimagelist[timestep, slice] = ft_tmp.copy()
...
def ddm_processing(binsize, analysisradius, cutoff, images_to_load, z_slices ,image_directory, file_prefix, file_suffix):
    """If calling functions from within python this is the main loop."""

    # Load the images
    ftimagelist, numimages, tmp_file = loadimages(images_to_load, analysisradius, image_directory, file_prefix, file_suffix, z_slices)
